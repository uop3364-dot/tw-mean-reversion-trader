...
def build_signals(df:pd.DataFrame,cfg:dict,calendar=None) -> pd.DataFrame:
    ...
    d=expanding_probability(add_features(clean,cfg),cfg); s=cfg["strategy"]; u=s["universe"]; o=s["oscillation"]; l=s["low_zone"]; m=s["mean_reversion"]; r=s["regime"]
    d["mean_reversion_score"]=(d.mr_probability*100).fillna(0)
    ...
    d["pass_liquidity"]=liquid;d["pass_oscillation"]=(d.oscillation_score>=o["minimum_score"]);d["pass_low_zone"]=(d.low_score>=l["minimum_score"])&(d.price_position<=l["max_price_position"])&(d.price_percentile<=l["max_price_percentile"])&(d.rsi14.between(l["rsi_minimum"],l["rsi_maximum"]))&(d.bb_position<=l["bollinger_max_position"])
    ...
    ev=historical_event_stats(d,cfg); d["take_profit"]=expanding_take_profit(d,ev,s["take_profit"]["candidates"],s["take_profit"]["fallback"])
    return d
